...
interface SubjectDefinition {
  ...
  inclined: (name: string) => { body: string; tip: string };
  support: (name: string) => { body: string; tip: string };
}
...
const SUBJECTS: SubjectDefinition[] = [
  {
    ...
    inclined: (name) => ({
      body: `Word problems, patterns, and step-by-step logic puzzles are likely to come more naturally to ${name} than you'd expect — the reasoning behind a method tends to interest them more than the method itself.`,
    ...
    support: (name) => ({
      body: `Straight arithmetic and memorised procedures — times tables, long division, formula recall — may take more repetition to stick for ${name}, even when their underlying reasoning is sound.`,
  ...
  },
  {
    ...
    score: (c) =>
      ...
      (houseEase(c, 2) - 5) * 0.4,
    inclined: (name) => ({
      body: `${name} is likely to enjoy reading beyond what's assigned, and to write with a natural, expressive voice once they're genuinely interested in the topic.`,
    ...
    support: (name) => ({
      body: `The mechanics of writing — spelling, grammar, getting an idea onto the page — may need more patience than the ideas themselves for ${name}, who may have plenty to say and find the writing-it-down part harder.`,
  ...
  },
  {
    ...
    inclined: (name) => ({
      body: `Hands-on experiments, "why does this happen" questions, and nature-based exploration are likely to hold ${name}'s attention far longer than a textbook chapter.`,
    ...
    support: (name) => ({
      body: `Abstract scientific ideas — invisible forces, chemical formulas, atomic structure — may need to be made concrete before they click for ${name}.`,
  ...
  },
  {
    ...
    score: (c) =>
      ...
      (houseEase(c, 9) - 5) * 0.3,
    inclined: (name) => ({
      body: `${name} is likely to connect with history and social studies through people and story rather than dates and facts — a natural curiosity about "why things are the way they are" often shows up here.`,
    ...
    support: (name) => ({
      body: `Memorising dates, names, and sequences of events in isolation may feel dry and hard to hold onto for ${name}.`,
  ...
  },
  {
    ...
    inclined: (name) => ({
      body: `There's a good chance ${name} takes to the step-by-step, trial-and-error logic of coding and building things digitally — it rewards exactly the patient problem-solving this chart tends to support.`,
    ...
    support: (name) => ({
      body: `The abstract, symbolic side of programming — syntax, debugging an error you can't see — may be genuinely frustrating for ${name} without a lot of hands-on guidance at first.`,
  ...
  },
  {
    ...
    inclined: (name) => ({
      body: `Art, design, and hands-on making are likely to be a genuine source of joy and confidence for ${name}, not just a break from "real" schoolwork.`,
    ...
    support: (name) => ({
      body: `Art may not come as naturally or confidently to ${name} as other areas, and that's completely fine — it doesn't need to be a strength to be worthwhile.`,
  ...
  },
  {
    ...
    inclined: (name) => ({
      body: `${name}'s chart suggests a natural feel for rhythm, melody, or emotional expression through sound.`,
    ...
    support: (name) => ({
      body: `Formal music training — reading notation, structured practice — may need more patience from ${name} than a natural feel for music would suggest.`,
  ...
  },
  {
    ...
    inclined: (name) => ({
      body: `${name} is likely to enjoy being heard — a class presentation or small stage moment can become a genuine source of pride rather than nerves.`,
    ...
    support: (name) => ({
      body: `Speaking up in front of a group may not come easily to ${name} at first, and that's worth respecting rather than pushing past too quickly.`,
  ...
  },
  {
    ...
    inclined: (name) => ({
      body: `Physical activity is likely to be a genuine strength and outlet for ${name}, not just recess — it can build real confidence that carries over into the classroom.`,
    ...
    support: (name) => ({
      body: `Structured sport — rules, coordination, competition — may take more patience for ${name} to enjoy than free play does.`,
...
export function buildSubjectGuidance(
  ...
  const inclined = ranked.slice(0, 4).map(({ subject }) => {
    const copy = subject.inclined(childName);
  ...
  const support = ranked
    ...
    .reverse()
    .map(({ subject }) => {
      const copy = subject.support(childName);
      return { id: subject.id, name: subject.name, body: copy.body, tip: copy.tip };
    });
