...
**Written:** 2026-08-26 · **Last updated:** 2026-08-31 · **Repo:** `vedicastro-alt/AstroEducation` · **Production branch:** `claude/vedic-horoscope-learning-site-fb6fta` · **Latest commit on this working branch (`claude/handoff-task-review-umvc5i`):** `97232f2`

> **New agent starting now: read §22 first.** It's the current top-priority task — the reading engine's content now needs a consistency/repetition audit after the career-significator work in §21, and §22 lays out exactly what to check and why. §18-21 are settled/background (§18's original conversion-test finding is fixed; §21 is what was just built on top of it).

> ⚠️ `claude/vedic-horoscope-learning-site-fb6fta` is this repo's default/HEAD branch — Vercel deploys straight from it, and there's no staging step, so pushing there is shipping to production immediately. **All of §21 and §22's work happened on a separate working branch, `claude/handoff-task-review-umvc5i`, which as of this writing has NOT been merged/pushed to production** — check `git log` on both branches before assuming which one has the latest code. Two large reference PDFs were also briefly pushed to production by accident and have since been removed cleanly (see §21) — if you see other stray branches with PDFs in `src/lib/education/` (e.g. `claude/handoff-context-review-8pf8w6`), those are source material for §21's work, not app code, and shouldn't be merged.
...
- §22 — **read this first.** The scoring got a lot deeper in §21 without a matching audit of whether the prose built on it is still internally consistent — concrete, verified examples of subject-vs-career-field score divergence, plus an audit methodology to run before doing anything else.
